...
$app->post('/item', function ($request, $response) {
    $input = $request->getParsedBody();
    $sql = "INSERT INTO item (itemid,name,price,status) VALUES (:itemid,:name,:price,:status)";
    $sth = $this->db->prepare($sql);
...
$app->post('/booking', function ($request, $response) {
    $input = $request->getParsedBody();
    $sql = "INSERT INTO booking (bookingid,fromdate,todate,customer,phone,address,email,numberOfPpl,allItems,allHall,damage,gst,service_tax,total,paid,invoice_num,gst_num) VALUES (:bookingid,:fromdate,:todate,:customer,:phone,:address,:email,:numberOfPpl,:allItems,:allHall,:damage,:gst,:service_tax,:total,:paid,:invoice_num,:gst_num)";
    $sth = $this->db->prepare($sql);
    ...
    $sth->bindParam("damage", $input['damage']);
    $sth->bindParam("gst", $input['gst']);
    $sth->bindParam("service_tax", $input['service_tax']);
...
$app->put('/booking/[{id}]', function ($request, $response, $args) {
    $input = $request->getParsedBody();
    $sql = "UPDATE booking SET bookingid=:bookingid,fromdate=:fromdate,todate=:todate,customer=:customer,phone=:phone,address=:address,email=:email,numberOfPpl=:numberOfPpl,allItems=:allItems,allHall=:allHall,damage=:damage,gst=:gst,service_tax=:service_tax,total=:total,paid=:paid,invoice_num=:invoice_num,gst_num=:gst_num WHERE id=:id";
    $sth = $this->db->prepare($sql);
    $sth->bindParam("id", $args['id']);
    $sth->bindParam("bookingid", $input['bookingid']);
    $sth->bindParam("fromdate", $input['fromdate']);
